---
name: deep-dive
description: Principal-level technical deep dive using Feynman Technique. User explains before artifacts. Per-topic MISSION.md, learning records, resources, glossary. Produces overview.md, cheatsheet.md, reflection.md. Use for teach-me, explain X, learn X, or /teach sessions.
---
...
Teach a topic at principal depth through retrieval, explanation, and gap-filling — then persist artifacts. Ground every session in per-topic `MISSION.md` and one micro-scope per session.
...
5. Read existing artifacts if present: `MISSION.md`, `overview.md`, `glossary.md`, `resources.md`, `learning-records/`
6. If `overview.md` exists — **revisit**; start with `/recall`-style retrieval (skip to step 2 of Flow)
...
Prefer sources from `resources.md`; if empty, suggest high-trust anchors and plan to append `resources.md` after the session.
...
- Do not write learning artifacts until the Feynman gate (step 4) is complete
